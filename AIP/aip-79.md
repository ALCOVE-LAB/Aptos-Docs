...
*缓解措施*：验证者可以强制进行时纪元更改并禁用随机性功能。在调试之后，Aptos Labs 需要部署修复补丁以实现全面的缓解。

**随机性生成失去活性**。如果发生这种情况，区块链将停止，因为每个区块都需要随机性才能继续执行。

*缓解措施*：验证者可以使用本地配置覆盖禁用随机性功能，并重新启动所有验证者以暂停随机性生成，以使链暂时恢复活动状态。在调试之后，Aptos Labs 需要部署修复补丁以实现全面的缓解。
...
**最低 gas 费**。如果用户的交易需要随机性，那么需要设置 `max_gas=10000`，原因是[我们已经实现了防止低 gas 攻击](#undergasing-attack)。为了支持这个功能，SDK 需要进行相应的更新。未来，我们应该允许每个 dApp 自行设定它的最低费用金额，而不是统一使用一个固定值。
